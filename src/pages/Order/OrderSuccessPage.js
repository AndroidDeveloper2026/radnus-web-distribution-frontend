...
const OrderSuccessPage = () => {
  ...
  const goToInvoice = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        paymentMode: selectedPaymentMode?.label || initialPaymentMode || "Cash",
        status: "completed",
        ...
        sameAsBuyer,
        shippingAddress: {
          name: finalShipToName || customer?.name || "Guest",
          phone: finalShipToPhone || buyerPhone || "",
          address: finalShipToAddress || customer?.address || "",
          city: finalShipToCity || customer?.city || "",
      ...
      if (err?.response?.data?.message) {
        ...
        if (err.response.data.code === "INSUFFICIENT_STOCK") {
          errorMessage = `Insufficient stock: ${err.response.data.message}`;
        }
      } else if (err?.message) {
  ...
  const handleViewInvoice = () => {
    ...
    if (!orderType) {
      alert("Please select an Order Type before proceeding.");
      return;
    }
    ...
    <div className={`order-success-page ${isDark ? "dark" : ""}`}>
      <div className="order-success-container">
        ...
        <div className="form-card">
          ...
          {/* Order Type */}
          <div className="field-group" ref={orderTypeDropdownRef}>
            <label>
            ...
            >
              <span>{orderType ? orderType : "Select order type…"}</span>
              <ChevronDown
                size={16}
                className={`dropdown-arrow ${orderTypeDropdownOpen ? "open" : ""}`}
              />
            </div>
            {orderTypeDropdownOpen && (
              <div className="dropdown-list">
                {ORDER_TYPES.map((type) => (
                  <div
                    key={type.id}
                    className={`dropdown-item ${orderType === type.label ? "active" : ""}`}
                    onClick={() => {
            ...
            {!orderType && (
              <div className="error-text">Order Type is required</div>
            )}
          </div>
...
//   // ─── FIXED: goToInvoice with proper batch allocations ──────────────────────────
...
//     try {
//       // ✅ STEP 1: Build invoice items with proper batch allocations
//       const invoiceItems = cartItems.map((item) => {
//         // Get batch allocations from the passed data
//         let batchAllocations = [];
        
//         // Option 1: From batchSelections (new format)
//         if (batchSelections && batchSelections[item.id]?.batchAllocations) {
//           batchAllocations = batchSelections[item.id].batchAllocations;
//         }
        
//         // Option 2: From item.batchAllocations (fallback)
//         if (batchAllocations.length === 0 && item.batchAllocations) {
//           batchAllocations = item.batchAllocations;
//         }
        
//         // ✅ Ensure we have valid batch allocations
//         if (batchAllocations.length === 0) {
//           throw new Error(`Missing batch allocation for ${item.name}`);
//         }
...
//           qty: item.qty,
//           price: item.price || 0,
//           batchAllocations: batchAllocations.map((alloc) => ({
//             batchNumber: alloc.batchNumber,
//             qty: alloc.qty,
//             purchaseCost: alloc.purchaseCost || 0
//           }))
//         };
//       });

//       // ✅ STEP 2: Verify all items have batch allocations
//       const missingAllocations = invoiceItems.filter(
//         (item) => !item.batchAllocations || item.batchAllocations.length === 0
//       );
//       if (missingAllocations.length > 0) {
//         throw new Error(
//           `Missing batch allocations for: ${missingAllocations.map((i) => i.name).join(", ")}`
//         );
//       }
...
//       // ✅ STEP 5: Prepare invoice payload
//       const payload = {
//         billerName: user?.name || "Unknown",
//         items: invoiceItems,
//         totalAmount: totalWithCourier,
...
//       console.log("📤 Sending invoice with batch allocations:", JSON.stringify(payload, null, 2));

//       const invoiceRes = await API.post("/api/invoices", payload);
//       const invoiceNumber = invoiceRes.data.invoice.invoiceNumber;

//       // ✅ STEP 7: Navigate to invoice page
//       navigate("/invoice", {
//         state: {
//           invoiceNumber,
//           items: cartItems,
...
//             "One or more items no longer have enough stock. Please review the cart."
//         );
//       } else {
...
//         {/* ✅ FIXED: Batch Summary - Properly render batch allocations */}
//         {showBatchSelector && (
//           <div className="batch-summary-card">
//             <h4>Batch Allocations</h4>
//             {cartItems.map((item) => {
...
//               // Try from batchSelections
//               if (batchSelections && batchSelections[item.id]?.batchAllocations) {
//                 allocations = batchSelections[item.id].batchAllocations;
//               }
              
//               // Try from item.batchAllocations
//               if (allocations.length === 0 && item.batchAllocations) {
//                 allocations = item.batchAllocations;
//               }
...
//                     {allocations.map((alloc, idx) => (
//                       <span key={idx} className="batch-summary-tag">
//                         {alloc.batchNumber}: {alloc.qty} units
//                       </span>
//                     ))}
//                   </div>
//                 </div>
...
//               {/* ✅ FIXED: Confirm modal batch summary */}
//               {showBatchSelector && (
//                 <div className="confirm-batch-summary">
//                   <h4>Batch Allocations</h4>
//                   {cartItems.map((item) => {
...
// //       // NOTE: stock is no longer reduced separately here. The backend now
// //       // consumes FIFO batches (StockBatch.quantityAvailable) AND decrements
// //       // Product.moq atomically inside the /api/invoices transaction itself,
// //       // so calling reduceStock here would double-decrement stock. If the
// //       // invoice POST above throws (e.g. insufficient stock, HTTP 409),
// //       // nothing was touched — the catch block below handles that.
...
// //       console.error(err);
// //       if (err?.response?.status === 409 && err?.response?.data?.code === "INSUFFICIENT_STOCK") {
// //         alert(err.response.data.message || "One or more items no longer have enough stock. Please review the cart.");
// //       } else {
// //         alert("Failed to confirm order. Please try again.");
// //       }
// //     } finally {
...
// //           </div>
// //         )}
// //         {referenceNo.trim() && !orderType && (
// //           <div
// //             className="error-text"
// //             style={{ textAlign: "center", marginTop: "0.5rem" }}
// //           >
// //             * Order Type is required to view invoice
// //           </div>
// //         )}
// //       </div>
...
// //   const paymentDropdownRef = useRef();
// //   const salespersonDropdownRef = useRef();
// //   const orderTypeDropdownRef = useRef(); // ✅ NEW
