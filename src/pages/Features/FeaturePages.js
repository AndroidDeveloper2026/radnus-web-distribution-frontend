...
export const TerritoryPage = () => {
  ...
  const handleSave = async () => {
    ...
    try { await dispatch(addTerritory({ state: vals.state, districts })).unwrap(); setModal(false); toast.success('Territory added'); }
  ...
  return (
    <div>
      <SectionHeader title="Territory Management" action={canManage && <Button variant="primary" size="sm" onClick={() => setModal(true)}>+ Add Territory</Button>} />

      {loading ? <div style={{ display:'flex', justifyContent:'center', padding:60 }}><Spinner size="lg" /></div>
        : list.length === 0 ? <EmptyState icon={<Map size={40}/>} title="No territories configured" subtitle="Add states, districts and taluks" action={canManage && <Button variant="primary" size="sm" onClick={() => setModal(true)}>+ Add Territory</Button>} />
        : list.map(t => (
          <div key={t._id} className="territory-node">
            <div className="territory-header" onClick={() => toggleExpand(t._id)}>
              ...
              <Badge variant="info">{t.districts?.length || 0} districts</Badge>
              <div style={{ marginLeft: 'auto', color: 'var(--text-muted)' }}>{expanded[t._id] ? <ChevronDown size={16}/> : <ChevronRight size={16}/>}</div>
              {canManage && <Button variant="secondary" size="xs" onClick={e => { e.stopPropagation(); dispatch(deleteTerritory(t.state)).then(() => toast.success('Removed')); }}>Remove</Button>}
            </div>
