...
const ExcelExportScreen = () => {
  ...
  const [orderTypeFilter, setOrderTypeFilter] = useState(''); // ← ADDED
  const [uniqueSalespersons, setUniqueSalespersons] = useState([]);
  const [uniqueOrderTypes, setUniqueOrderTypes] = useState([]); // ← ADDED
  ...
  // Extract unique order types ← ADDED
  useEffect(() => {
    if (invoices && invoices.length > 0) {
  ...
  const applyAllFilters = useCallback((data, reportType) => {
    ...
    filtered = filterDataByOrderType(filtered); // ← ADDED
      ...
      <div className="main-content">
          ...
          <div className="order-type-filter-container">
            ...
            <div className="order-type-buttons">
              ...
              {uniqueOrderTypes.map(ot => (
                <button
                  key={ot}
                  className={`order-type-btn ${orderTypeFilter === ot ? 'active' : ''}`}
                  onClick={() => setOrderTypeFilter(ot)}
                  style={{
                    backgroundColor: orderTypeFilter === ot ? 
                      (ot === 'OEM' ? '#E3F2FD' : '#FFF3E0') : 'transparent',
                    color: orderTypeFilter === ot ? 
                      (ot === 'OEM' ? '#1565C0' : '#E65100') : 'inherit',
                    borderColor: orderTypeFilter === ot ? 
                      (ot === 'OEM' ? '#90CAF9' : '#FFCC80') : '#e0e0e0'
                  }}
                >
                  {ot}
                </button>
        ...
        {(salespersonFilter || searchTerm || periodFilter !== 'all' || orderTypeFilter) && (
          <div className="active-filters">
            ...
            {orderTypeFilter && ( // ← ADDED
              <span className="active-filter-tag" style={{
                backgroundColor: orderTypeFilter === 'OEM' ? '#E3F2FD' : '#FFF3E0',
                color: orderTypeFilter === 'OEM' ? '#1565C0' : '#E65100'
              }}>
...
//   const filterDataBySalesperson = useCallback((data, reportType) => {
//     if (!salespersonFilter || salespersonFilter === '') return data;
    
//     if (reportType === 'invoices' || reportType === 'salesReturns') {
//       return data.filter(item => item.salesperson === salespersonFilter);
//     }
...
//     return filtered;
//   }, [filterDataByDate, filterDataBySalesperson, searchTerm]);
...
//     setSelectedModal(reportId);
//     setCurrentPage(1);
//   }, [getRawData, applyAllFilters, salespersonFilter]);
