...
export const StatCard = ({ icon, label, value, delta, deltaType = 'up', accent = 'red', onClick }) => {
  ...
  const valueColor = isDark ? '#f5f5f7' : '#0f0f10';
  const labelColor = isDark ? '#a1a1aa' : '#6b7280';
  const deltaColor = deltaType === 'up' ? (isDark ? '#4ade80' : '#16a34a') : '#ef4444';

  return (
    <div
      className={`stat-card stat-${accent} ${onClick ? 'stat-clickable' : ''}`}
    ...
    >
      ...
      {delta && (
        <div className={`stat-delta delta-${deltaType}`} style={{ color: deltaColor }}>
          {delta}
        </div>
      )}
    </div>
  ...
  return (
    <div 
      ...
      style={{
        ...
        borderRadius: isDark ? 'var(--radius)' : '0',
        padding: isDark ? '48px 20px' : '48px 20px',
        border: isDark ? '1px solid #2a2a2a' : 'none'
      }}
    >
      {icon && <div className="empty-icon">{icon}</div>}
      <div className="empty-title" style={{ color: isDark ? '#f5f5f7' : 'var(--text-secondary)' }}>
        {title}
      </div>
      {subtitle && (
        <div className="empty-sub" style={{ color: isDark ? '#a1a1aa' : 'var(--text-muted)' }}>
          {subtitle}
        </div>
      )}
